...
def video_has_audio_track_task(input_path: str) -> FFprobeTask:
    return FFprobeTask(
        global_options=["-v error"], inputs={input_path: "-show_streams -select_streams a -loglevel error"}
    )
...
def add_audio_track_task(input_path: str, output_path: str) -> FfmpegTask:
    return FfmpegTask(
        ...
        inputs={"aevalsrc=0": None, input_path: None},
...
def video_to_video(input_path: str, output_path: str, video_settings: Optional[GifSettings] = None) -> List[FfmpegTask]:
    if not video_settings:
        return [FfmpegTask(inputs={input_path: None}, outputs={output_path: "-qscale 0"})]
...
def single_pass_convert(input_path: str, output_path: str, video_settings: GifSettings) -> FfmpegTask:
    ...
    return FfmpegTask(inputs={input_path: None}, outputs={output_path: ffmpeg_args})
...
def two_pass_convert(input_path: str, output_path: str, video_settings: GifSettings) -> List[FfmpegTask]:
    ...
    task1 = FfmpegTask(global_options=["-y"], inputs={input_path: None}, outputs={os.devnull: two_pass_args[0]})
    task2 = FfmpegTask(global_options=["-y"], inputs={input_path: None}, outputs={output_path: two_pass_args[1]})
    return [task1, task2]
