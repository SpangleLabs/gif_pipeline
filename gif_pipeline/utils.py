...
async def tqdm_gather(awaitables: List[Awaitable[T]]) -> List[T]:
    ...
    numbered_results = [await f for f in tqdm(asyncio.as_completed(numbered_awaitables), total=len(awaitables))]
