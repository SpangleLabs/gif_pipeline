...
- `website`: `Website` (optional), website configuration, see below for details. If provided, the channel will be available on the website.
...
Configuration for how the channel will be (or won't be) available on the website
- `enabled`: `boolean` (optional, default: "true") Defaults false if no website configuration is set, but if any website configuration is provided, this will default to false. Whether the channel has a page available on the website
- `publicly_listed`: `boolean` (optional, default: "false") Whether to list the channel on the index home page.
...
- `frigate`: Details for a Frigate cctv webserver
  - `frigate.url`: The base web URL for the frigate webserver, including protocol and port
...
### Download helper
Scans messages for links, and will attempt to download them using the [Youtube Downloader](https://github.com/yt-dlp/yt-dlp) tool. Skips certain links that specialised handlers can handle, such as fa and msg handlers. If the link is to a playlist, it will only download the first video of the list.
...
### Find helper
This handler will attempt to find a video in a playlist, which matches the video which the user is replying to.
Use `find {playlist link}` and it will search the playlist for a matching video, and respond with the video and link.

### Frigate helper
This handler is to export and download video clips from a Frigate web server (see https://frigate.video).
Use `frigate {camera_name} {start_time} {end_time}` to specify which camera clip to export.
